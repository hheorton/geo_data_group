...
#             for mn in range(self.periods):
    ...
    def clim_centile(self,centiles,mask = False,full_time = False,year_set = [],time_set = [],method='mean',mult_array=False,first_p = 0):
        """
        Mask needs to be 1 for true 0 for false
        """
        # check if there's data here already
...
#             print(idx)
    ...
    def save_nc(self,filename,d0,tdim = 'days',mask = False,grid = False,
                ...
                description='default data_year ',data_name = 'dy_data',
                      year_set = [],time_set = [],verbos = False):
        ...
        if tdim == 'seconds':
            temp_time = [(self.dates[t]-d0).seconds for t in t_use]
        ...
        DY_time = NC_f.createVariable('time_point', 'f4', ('time',))
        if save_yrpd:
...
#         DY_yrpd_mask = NC_f.createVariable('dy_yrpd_mask', 'i1', ('nyrs','periods'))
        ...
        if tdim == 'days':
            NC_f.setncattr_string('Time dimension','days since '+d0.strftime('%Y-%m-%d'))
        if tdim == 'seconds':
            NC_f.setncattr_string('Time dimension','days since '+d0.strftime('%Y-%m-%dT%H:%M:%S.%f'))
